...
\centering
\includegraphics[width=\textwidth]{figures/fig_teaser_a.pdf}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.48\textwidth}
\centering
\includegraphics[width=\textwidth]{figures/fig_teaser_b.pdf}
\end{subfigure}
...
This paper documents the \emph{probe reward trap}: when a periodically retrained linear probe on $\dsyc$ is used as a non-differentiable Group Relative Policy Optimization (GRPO) reward signal, behavioral death (true-positive flip rate, $\tof = 0$) occurs within 500 steps while generation entropy remains healthy, masking the failure from standard monitoring (\S\ref{sec:probe_trap}).
The 1{,}300-step lag before distributional collapse means entropy-based early stopping, the most common safeguard against reward hacking \citep{casper2023openproblems}, would miss the failure entirely.
The trap arises even when the probe reward constitutes only a fraction of the total reward signal, reflecting a limitation of $\dsyc$ itself: the direction encodes whether belief changed, not whether it should have, so any reward conditioned on it alone will either be gamed or reward rigidity.

The failure is not specific to probe rewards.
Under our GRPO configuration (Qwen3-8B, group size$=$2), cosine projection and continuous probe formulations of $\dsyc$ also collapse, and removing $\dsyc$ entirely does not help: behavioral-reward-only GRPO rarely exceeds vanilla across seeds.
By contrast, Activation Consistency Training \citep[ACT;][]{consistency2025bctact}, which provides dense per-token supervision rather than a single scalar per completion, exceeds vanilla at every evaluated checkpoint (\S\ref{sec:discussion}).
...
\begin{itemize}
\item \textbf{Observed reliability gap between supervision paradigms} (C1): per-token supervised fine-tuning (SFT) exceeds vanilla at every evaluated checkpoint across two seeds, including a reduced-LoRA ablation.\ Per-completion GRPO rarely exceeds vanilla in any configuration (Table~\ref{tab:main_comparison}). Under matched LoRA, per-token SFT achieves $2.1{\times}$ higher peak sycophancy calibration score ($\sycon$; \S\ref{ssec:eval}) than per-completion GRPO (\S\ref{sec:discussion}).
...
\item \textbf{Shared root cause across formulations} (C3): $\dsyc$ encodes whether belief changed, not whether it should have; valid revision and sycophantic capitulation produce overlapping activation signatures \citep{wang2025internal, shapira2026amplifies}. Natural language inference (NLI) behavioral rewards exhibit the same warrant blindness, explaining why removing $\dsyc$ does not recover calibration (\S\ref{sec:discussion}).
\end{itemize}
