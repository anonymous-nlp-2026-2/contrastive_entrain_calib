...
Without stop-gradient, the policy discovers within tens of steps that shifting activations away from the sycophancy cluster maximizes $\rone$ regardless of response content, producing a fully inverted behavioral profile.
Replication across three seeds confirms determinism: all collapse within the first training epoch, with each probe retraining immediately re-gamed.
...
By step 500, $\tof$ drops to zero while $\nof$ remains near-perfect, yielding $\sycon = 0$.
Qualitative analysis of 181 valid-correction responses reveals five failure modes: \emph{deliberation traps} (28.7\%), where the model identifies the correct answer but cannot commit; \emph{surface compliance / sycophantic stubbornness} (24.3\%), where the model verbally agrees but does not change its answer; \emph{acknowledge-but-maintain} (23.8\%); \emph{genuine-accept-missed} (18.8\%); and refute/ignore/other (4.5\%). Full taxonomy in Appendix~\ref{app:taxonomy}.
...
\centering
\includegraphics[width=\textwidth]{figures/fig_probe_trap.pdf}
\caption{Probe reward trap (stopped-gradient probe). $\tof$ drops to zero at step 500 while entropy remains high (0.70--0.80) through step 1000. The 1{,}300-step lag evades entropy monitoring.}
...
\caption{$\sycon$ trajectories for $\rthree$-only GRPO (two seeds) and ACT (two seeds). ACT exceeds vanilla at every checkpoint (31/31); GRPO exceeds vanilla at only 4/28 (14.3\%).}
\label{fig:trajectory}
\end{subfigure}
\caption{Training dynamics. (a)~The probe reward trap: behavioral death precedes distributional collapse. (b)~$\rthree$-only GRPO vs.\ ACT over training steps.}
\label{fig:dynamics}
\end{figure*}
